...
def test_send_reminders_intro(client, mock_templated_email):
...
def test_send_reminders_submit(client, mock_templated_email):
...
def test_send_reminders_no_applications_to_remind(client, mock_templated_email):
    client.post("/create-test-application", json=application_payload)
    send_reminders()
