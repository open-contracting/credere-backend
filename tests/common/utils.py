...
def create_enums(engine):
    ...
    if not enum_exists:
        BorrowerSizeEnum = Enum(
            *borrower_size_values, name="borrower_size", create_type=False
        )
        engine.execute(
    ...
    if not enum_exists:
        CreditTypeEnum = Enum(
            *credit_type_values, name="credit_type", create_type=False
        )
    ...
    if not enum_exists:
        UserTypeEnum = Enum(*user_type_values, name="user_type", create_type=False)
