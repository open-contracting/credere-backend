...
def get_or_create_borrower(entry):
    borrower_identifier = background_utils.get_secret_hash(entry.get("nit_entidad"))
    if borrower_identifier in borrowers_list:
        return get_borrower_id(borrower_identifier)
    borrower_url_email = (
        f"{background_config.BORROWER_EMAIL_URL}?nit={entry['documento_proveedor']}"
    )
    borrower_response_email = httpx.get(
        borrower_url_email, headers=background_config.headers
    )
    borrower_response_email_json = borrower_response_email.json()[0]

    if not re.match(
        background_config.pattern, borrower_response_email_json["correo_entidad"]
    ):
        raise sentry_sdk.capture_exception("Borrower has no valid email address")

    borrower_url = (
        f"{background_config.BORROWER_URL}?nit_entidad={entry['documento_proveedor']}"
    )
