...
class Application(SQLModel, table=True):
    ...
    status: ApplicationStatus = Field(
        sa_column=Column(SAEnum(ApplicationStatus, name="application_status")), default=ApplicationStatus.PENDING
    )
...
class Borrower(SQLModel, table=True):
    ...
    status: BorrowerStatus = Field(
        sa_column=Column(SAEnum(BorrowerStatus, name="borrower_status")), default=BorrowerStatus.ACTIVE
    )
    created_at: Optional[datetime] = Field(
        sa_column=Column(DateTime(timezone=True), nullable=False, server_default=func.now())
    )
...
class Award(SQLModel, table=True):
    ...
    source_last_updated_at: Optional[datetime] = Field(sa_column=Column(DateTime(timezone=True), nullable=False))
